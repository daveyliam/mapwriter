...
public class Texture {
	...
	public synchronized void getRGB(int x, int y, int w, int h, int[] pixels,
			int offset, int scanSize, String Blockname) {
		int bufOffset = (y * this.w) + x;
		for (int i = 0; i < h; i++) {
			if (bufOffset + (i * this.w) <= this.pixelBuf.limit()) {
				this.pixelBuf.position(bufOffset + (i * this.w));
				this.pixelBuf.get(pixels, offset + (i * scanSize), w);
			} else {
				MwUtil.log(
						"MwTexture.getRGB: buffer over limit (limit %d)(request %d)(Block %s)",
						this.pixelBuf.limit(), bufOffset + (i * this.w),Blockname);
			}
